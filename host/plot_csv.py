...
parser.add_argument("--dmm", type=float, default=None, help="reference DMM VBATT in volts (optional)")
...
if not vbatt:
raise SystemExit("No data parsed — check CSV path and headers: timestamp,elapsed_s,vbatt_v")


v_mean = mean(vbatt)
print(f"Samples: {len(vbatt)} Mean VBATT: {v_mean:.4f} V Min: {min(vbatt):.4f} Max: {max(vbatt):.4f}")
